...
-   **Linting:** Run `pylint` or `flake8`/`ruff` on your code to catch bugs and style issues.
-   **Imports:** Use `import x` for packages/modules. Use `from x import y` only
    when `y` is a submodule or class/function.
...
-   **Type Annotations:** Strongly encouraged for all public APIs and Pydantic models.
...
-   **Strings:** Use f-strings for formatting. Be consistent with double (`"`) quotes.
-   **Imports Formatting:** Imports should be on separate lines and grouped:
    standard library, third-party, and application imports.
...
-   All executable files should have a `main()` function or run under Uvicorn.
